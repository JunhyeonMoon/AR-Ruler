...
Java_com_example_whgns_ex1_MainActivity_ConvertRGBtoGray(JNIEnv *env, jobject instance,
                                                         jlong matAddrInput, jlong matAddrResult, jint a, jint b, jint c, jfloat x1, jfloat y1, jfloat x2, jfloat y2) {
    ...
    s_x = x1 * x_rate;
    s_y = y1 * y_rate - (float)22;
    e_x = x2 * x_rate;
    e_y = y2 * y_rate - (float)22;
    ...
    Point2f p3 = {e_x, e_y};
    Point2f p4 = {s_x, e_y};
    Point2f p5 = {0.0f, 0.0f};
    Point2f p6 = {110.2f, 110.2f};
    ...
    if(contours.size() > 0) {
        for (int i = 0; i < contours.size(); i++) {
            if (contours[i].size() > 500 && contours[i].size() < 4000) {
